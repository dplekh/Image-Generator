 ...
 */
public class HorizontalStripesImageGenerator implements ImageGenerator
{

	private static final String FOLDER = "random/hStripes/";
	private static final String FILE_EXTENSION = ".jpg";
	private static final int DIVISOR = 10;

	public static void main(String[] args) throws IOException
	{
		ImageGenerator generator = new HorizontalStripesImageGenerator();
		int size = 500;
		for (int i = 0; i < 100 ; i++)
		{
	...
	public void generate(final int size, final String filename) throws IOException
	{
		...
		for (int i = 0; i < size; i+=step)
		{
			for (int j = 0; j < size; j+=step)
			{
				g.setColor(colors[chooseColor(j, step)]);
				g.fillRect(i, j, step, step);
			}
		}
	...
	private int chooseColor(int n, int step) {
		return n/step%2 == 0?0:1;
	}
}
