 ...
 */
public class VerticalStripesImageGenerator implements ImageGenerator
{

	// generateImage(size, color, type, path);

	private static final String FOLDER = "random/vStripes/";
	private static final String FILE_EXTENSION = ".jpg";
	private static final int DIVISOR = 10;

	public static void main(String[] args) throws IOException
	{
		ImageGenerator generator = new VerticalStripesImageGenerator();
		int size = 500;
		for (int i = 0; i < 100 ; i++)
		{
	...
	public void generate(final int size, final String filename) throws IOException
	{
		...
		for (int i = 0; i < size; i+=step)
		{
			for (int j = 0; j < size; j+=step)
			{
				g.setColor(colors[chooseColor(i, step)]);
				g.fillRect(i, j, step, step);
			}
		}
